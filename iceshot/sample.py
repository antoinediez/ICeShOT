...
def sample_uniform(N,d,dtype=torch.float32,device="cuda"):
...
def sample_Aorientations_mat(N,r=3.0,dtype=torch.float32,device="cuda"):
...
def sample_cropped_domain(crop_function,n,dim=2,dtype=torch.float32,device="cuda"):
    grid_points = sample_grid(n,dim=dim,dtype=dtype,device=device)
    return grid_points[crop_function(grid_points)>0,:]

def sample_cropped_crystals(crop_function,N,dim=2,dtype=torch.float32,device="cuda"):
